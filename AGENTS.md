...
- **Settings UI**: `components/ToleranceControl.tsx` exports two components rendered as separate panels in `App.tsx` — `ModelSettings` (model selection, top) and `ToleranceControl` (output/threshold controls, bottom). `TaggerModelInfo.group` (renamed from `family`) groups models in the `<select>` `<optgroup>`s; the i18n key is still `settings.backend.families.*` (keyed by the `group` value, not renamed). `TaggingSettings.ignoreModelThresholds` is **client-side only** — it is not sent to the backend; `appendOutputParams` in `services/taggerService.ts` is the single source of truth for what `TaggingSettings` fields become query params.
- **Safebooru toggle**: `Results` takes a `useSafebooru` prop (default `true`) that switches the Danbooru artist-lookup host between `safebooru.donmai.us` and `danbooru.donmai.us`. Post-count and preview-fetch caches in `Results.tsx` are keyed by host, so toggling it refetches rather than serving stale entries.
- **Auto-fallback**: `BackendConfig.autoFallback` (client-side only, not sent to the backend) toggles automatic switching between the main backend (`MAIN_BACKEND_URL` in `App.tsx`, `https://localtagger.gpu.garden`) and the fallback (`FALLBACK_BACKEND_URL`, `https://mooshie-localtagger.hf.space`). Two effects in `App.tsx` handle it: on mount, if we're on the fallback and the main is back up, switch back; when `health` goes `down` while on the main, switch to the fallback if it's up. Custom addresses are never touched — both effects guard on `taggerBaseUrl === MAIN/FALLBACK_BACKEND_URL`. The checkbox lives in `ModelSettings` (`components/ToleranceControl.tsx`), left-aligned in the same row as the reset button.
- **Clickable form rows**: Konsta's `ListInput` only wires clicks on the input element itself — clicking the label text, padding, or media icon does nothing. The `focusRowInput` helper in `components/ToleranceControl.tsx` fixes this: it's an `onClick` handler that focuses the row's control (and opens the dropdown for `type="select"` via `showPicker()` where supported) when the click lands on anything but the control itself. Pass it to every `ListInput`.
